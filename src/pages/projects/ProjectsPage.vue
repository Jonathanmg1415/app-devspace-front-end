<template>
  <q-page padding>
    ...
    <div v-else-if="!projects.length" class="flex flex-center column q-py-xl" style="color:var(--ds-text-3)">
      ...
      <div v-for="project in projects" :key="project.id"
        ...
        <div style="padding:14px 14px 14px 18px">
          ...
          <div class="row items-center q-mt-sm" style="gap:6px">
            <span v-if="project._role === 'member'" class="ds-status-badge"
              style="background:rgba(56,189,248,0.12); color:#38BDF8; border:1px solid rgba(56,189,248,0.2)">
              miembro
            </span>
            <span v-else class="ds-status-badge active">
...
function resetForm() { editing.value = null; form.value = { name: '', description: '', color: '#F97316' } }

async function handleSubmit() {
  if (!form.value.name) return
  saving.value = true
  try {
    if (editing.value) { await store.update(editing.value.id, form.value); $q.notify({ type: 'positive', message: 'Proyecto actualizado' }) }
    else { await store.create(form.value); $q.notify({ type: 'positive', message: 'Proyecto creado' }) }
