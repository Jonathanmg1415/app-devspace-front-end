...
async function handleInvite() {
  ...
  try {
    ...
    if (status === 404) $q.notify({ type: 'warning', message: 'No existe ninguna cuenta con ese email' })
    else if (status === 409) $q.notify({ type: 'warning', message: 'Este usuario ya es miembro del proyecto' })
    else if (status === 400) $q.notify({ type: 'warning', message: 'No puedes invitarte a ti mismo' })
