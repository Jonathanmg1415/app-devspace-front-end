...
export const useNotesStore = defineStore('notes', () => {
  ...
  async function fetchAll(projectId) {
    ...
    loading.value = true
    try {
      const { data } = await api.get('/api/notes', { params: { projectId, limit: PAGE, skip: 0 } })
  ...
  async function loadMore() {
    ...
    try {
      const { data } = await api.get('/api/notes', { params: { projectId: _projectId, limit: PAGE, skip: _skip } })
